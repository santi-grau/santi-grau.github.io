...
var Composer = function( renderer, rendertarget, scene, camera, controller ){
	...
	for (var id in Shaders) {
		...
		if( this[id].uniforms.resolution ) this[id].uniforms.resolution.value = new THREE.Vector2( rendertarget.width, rendertarget.height );
		this[id].enabled = false;
		this.effects[id] = { mode : 0, enabled : false };
	}

	// activate invert by default
	// this.invert.uniforms.on.value = 0;
...
Composer.prototype.bar = function( ){
	...
	if( Math.random() > 0.6 ) this.RGBShift.enabled = true;
	...
	if( Math.random() > 0.5 ) this.pixel.enabled = true;
